...
class GlobalPointer(Layer):
    ...
    def call(self, inputs, mask=None):
        # 输入变换
        inputs = self.dense(inputs) # shape(batchsize,sentencelenth,1280)
        inputs = tf.split(inputs, self.heads, axis=-1) # 按1280这个维度切分，heads=10，就切分成10份，1280/10 = 128
        # inputs结果为一个list，inputs[i]的shape为(batchsize,sentencelenth,128)，共有10个元素
        inputs = K.stack(inputs, axis=-2) # inputshape为(16,sentencelenth,10,128)

        qw, kw = inputs[..., :self.head_size], inputs[..., self.head_size:]
        # 前64是qw，后64是kw，shape都是((16,sentencelenth,10,64))
        # RoPE编码
        if self.RoPE:
        ...
        logits = sequence_masking(logits, mask, '-inf', 2)  # mask掉矩阵的后面几行
        # 上面sequence_masking中mask扩展维度后为16 1 52 1,与logits做element-wise乘法，需要广播成16 10 52 52再乘
        # 看最后两个维度，52行，1列，就是把该列复制52份即可

        logits = sequence_masking(logits, mask, '-inf', 3)  # mask矩阵的掉后面几列
        # 上面sequence_masking中mask扩展维度后为16 1 1 52,与logits做element-wise乘法，需要广播成16 10 52 52再乘
        # 看最后两个维度，1行52列，就是把该行复制52份即可

        # self.my = mask  # 方便function调试
        # 排除下三角
        if self.tril_mask:
...
class Zhuanli(Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(Zhuanli, self).__init__(**kwargs)
        # self.heads = heads
        self.head_size = head_size
        self.RoPE = RoPE
        self.use_bias = use_bias
    ...
    def build(self, input_shape):
        super(Zhuanli, self).build(input_shape)
        self.dense1 = Dense(
            units=self.head_size*2,
            use_bias=self.use_bias,
            kernel_initializer=self.kernel_initializer,
            activation='tanh',
            # kernel_regularizer=regularizers.l2(0.01)  # L2正则
        )
        self.dense2 = Dense(
            units=self.head_size*2,
            use_bias=self.use_bias,
    ...
    def call(self, inputs, mask=None):
        ...
        # qw = self.dense3(self.dense1(inputs))
        # kw = self.dense4(self.dense2(inputs))
        qw = self.dense3(inputs)
        kw = self.dense4(inputs)
        # RoPE编码
        if self.RoPE:
            pos = SinusoidalPositionEmbedding(self.head_size, 'zero')(inputs)
            cos_pos = K.repeat_elements(pos[..., None, 1::2], 2, -1)
            ...
            kw = kw * cos_pos + kw2 * sin_pos
        # 计算内积
        logits = tf.einsum('bmh,bnh->bmn', qw, kw)

        # 排除padding
        logits = sequence_masking(logits, mask, '-inf', 2)
        logits = sequence_masking(logits, mask, '-inf', 1)
        ...
        if self.tril_mask:
            ...
            logits = logits - (1 - mask) * K.infinity()
        # scale返回
        # self.my = logits  # 方便function调试
        return Activation('sigmoid')(logits/self.head_size**0.5)
        # return logits/self.head_size**0.5
    ...
    def get_config(self):
        ...
        base_config = super(Zhuanli, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
